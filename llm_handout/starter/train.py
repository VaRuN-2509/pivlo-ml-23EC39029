...
def main():
    ...
    ap.add_argument("--batch", type=int, default=32) #batch_size from 8 to 16
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=max_lr, weight_decay=0.1, betas=(0.9, 0.95))

    # 2. Define the Cosine Warmup Schedule
    def get_lr(step, max_steps, peak_lr, warmup_steps):
    ...
    grad_accum_steps = 4  # 4 micro-steps of batch size 16 = effective batch of 64!
    
    for step in range(1, args.steps + 1):
        # Update LR for this step
        lr = get_lr(step, args.steps, max_lr, warmup_iters)
